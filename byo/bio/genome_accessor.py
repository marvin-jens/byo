...
class GenomeCache(metaclass=Singleton):
    ...
    def __init__(self,path):
        self.cached = {}
        self.path = path
        self.logger = logging.getLogger("byo.bio.genome_accessor.GenomeCache")
...
class TwoBitAccessor(Accessor):
    def __init__(self, path,chrom,sense,system=None, split_chrom="", **kwargs):
        ...
        self.logger = logging.getLogger('byo.bio.TwoBitAccessor({0})'.format(fname))
        t0 = time()
        try:
            self.data = TB.TwoBitFile(fname)
            # print "opening worked", fname
            t1 = time()
...
class IndexedFasta(object):
    def __init__(self,fname,split_chrom="",lz = False, **kwargs):
        self.logger = logging.getLogger('byo.bio.IndexedFasta')
        self.fname = fname
        self.chrom_stats = {}
        self.split_chrom = split_chrom

        if lz:
            from byo.bio.lz import LZFile
...
class GenomeAccessor(Accessor):
    def __init__(self,path,chrom,sense,system=None,**kwargs):
        ...
        self.logger = logging.getLogger('byo.bio.GenomeAccessor')
...
class MSFAccessor(Accessor):
    def __init__(self,path,chrom,sense,system='hg19',offset=0,suffix='.maf.stitched.cmpl.repeats_lc',**kwargs):
        ...
        self.logger = logging.getLogger('byo.bio.MSFAccessor')
