...
from byo.bio.lazytables import NamedTupleImporter as Importer
def gff_importer(path,fix_chr=True,**kwargs):
    cols = ['chrom','source','type','start','end','score','strand','frame','attr_str']
...
def format_gff(chrom,source,type,start,end,score,strand,frame,attr_str):
    out = [chrom,source,type,start,end,score,strand,frame,attr_str]
