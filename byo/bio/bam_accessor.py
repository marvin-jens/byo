...
class BAMAccessor(Accessor):
    def __init__(self,path,chrom,sense,sense_specific=False,dtype=numpy.uint32,mode="c",filter=None,dim=1,mult=True,unique=True, system='hg19', read_count=False, full_reads=False, only_starts=False,**kwargs):
    ...
    def get_raw(self, chrom, start, end, sense, **kwargs):
        pile = []
        for read in self.bam.fetch(chrom,max(0,start),end):
