...
Locus = namedtuple("Locus","name, chrom, strand, start, end, category")
...
class ExonChain(object):
    ...
    def __init__(self, chrom, strand, exon_starts, exon_ends, system=None, name=""):
        self.system = system
        self.chrom = chrom
        self.strand = strand
    ...
    def map_from_spliced(self,pos):
        #print pos,self.strand,self.exon_count,self.ofs,self.dir,self.spliced_length
    ...
    def splice(self,track,join = lambda l : "".join(l),get="get_oriented",**kwargs):
        ...
        return join([get_func(self.chrom,start,end,self.strand,**kwargs) for start,end in self.exon_bounds][::self.dir])
    ...
    def unspliced_sequence(self):
        return self._system.genome.get_oriented(self.chrom, self.start, self.end, self.strand)
    ...
    def exons(self):
        for i,(start,end) in enumerate(self.exon_bounds[::self.dir]):
            yield Locus("%s/exon.%02d" % (self.name,i+1), self.chrom, self.strand, start,end,"exon")

    @property
    def introns(self):
        for i,(start,end) in enumerate(self.intron_bounds[::self.dir]):
            yield Locus("%s/intron.%02d" % (self.name,i+1), self.chrom, self.strand, start,end,"intron")

    @property
    def introns_as_chains(self):
        for i,(start,end) in enumerate(self.intron_bounds[::self.dir]):
            yield ExonChain(self.chrom, self.strand, [start,], [end,], name="%s/intron.%02d" % (self.name,i+1), system=self.system)
    ...
    def intersect(self, names, start, end, expand=False):
        ...
        if chain_starts:
            chain = ExonChain(self.chrom, self.strand, chain_starts, chain_ends, system=self.system)
        else:
            chain = self.EmptyChain()

        if before_starts:
            before = ExonChain(self.chrom, self.strand, before_starts, before_ends, system=self.system)
        else:
            before = self.EmptyChain()
            
        if after_starts:
            after = ExonChain(self.chrom, self.strand, after_starts, after_ends, system=self.system)
        else:
    ...
    def __add__(self,other):
        ...
        return ExonChain(A.chrom, A.strand, exon_starts, exon_ends, system=A.system)
    
        
    # add some python magic to make things smooth
    def __str__(self):
        exonlist = ",".join(map(str,self.exon_bounds))
        return "{self.chrom}:{self.start}-{self.end}{self.strand} spliced_length={self.spliced_length} exons: {exonlist}".format(self=self, exonlist=exonlist)
    ...
    def bed12_format(self,color="255,0,0"):
        ...
        cols = [self.chrom, self.start, self.end, self.name, getattr(self,"score",0), self.strand, cds_start, cds_end, color, self.exon_count, ",".join(block_sizes), ",".join(block_starts)]
    ...
    def ucsc_format(self):
        ...
        out = (-1,self.name,self.chrom,self.strand,self.start,self.end,cds_start,cds_end,self.exon_count,exonstarts,exonends,getattr(self,"score",0),getattr(self,"gene_id",self.name), "unk","unk",exonframes)
    ...
    def EmptyChain(self):
        return ExonChain(self.chrom, self.strand, [0], [0], system=self.system)
...
class Transcript(ExonChain):
    ...
    def segments(self):
        if self.UTR5:
            UTR = self.UTR5
            yield Locus("%s/UTR5" % self.name,UTR.chrom,UTR.strand,UTR.start,UTR.end,"5UTR")

        if self.UTR3:
            UTR = self.UTR3
            yield Locus("%s/UTR3" % self.name,UTR.chrom,UTR.strand,UTR.start,UTR.end,"3UTR")

        if self.CDS:
            CDS = self.CDS
            yield Locus("%s/CDS" % self.name,CDS.chrom,CDS.strand,CDS.start,CDS.end,"CDS")
    ...
    def features(self):
        ...
        yield Locus("%s/TSS" % self.name,self.chrom,self.strand,start,start+1,"TSS")
        yield Locus("%s/PAS" % self.name,self.chrom,self.strand,end,end+1,"PAS")

        for i,(ss5,ss3) in enumerate(self.splice_sites):
            yield Locus("%s/intron.%02d/SS5" % (self.name,i+1),self.chrom,self.strand,ss5,ss5+1,"5'SS")
            yield Locus("%s/intron.%02d/SS3" % (self.name,i+1),self.chrom,self.strand,ss3,ss3+1,"3'SS")

        if self.CDS:
            cds_start,cds_end = [self.CDS.start,self.CDS.end][::self.dir]
            yield Locus("%s/CDSstart" % self.name,self.chrom,self.strand,cds_start,cds_start+1,"CDSstart")
            yield Locus("%s/CDSend" % self.name,self.chrom,self.strand,cds_end,cds_end+1,"CDSend")
...
class CircRNA(Transcript):
    ...
    def cut(self,start,end,mode="auto"):
        ...
        if mode == "outside" or (mode == "auto" and ((start > end and self.strand == '+') or (start < end and self.strand == '-')) ):
            # circRNA wrap-around!
            new_chain = before + after
            outside = CircRNA(new_name+"_outer", new_chain.chrom, new_chain.strand, new_chain.exon_starts, new_chain.exon_ends, [new_chain.start, new_chain.start], system=self.system)
            outside.wraparound = [before.spliced_length,after.spliced_length][::outside.dir]
            outside.set_origin(start)
            return outside
        else:
            new_chain = inside
            inside = CircRNA(new_name+"_inner", new_chain.chrom, new_chain.strand, new_chain.exon_starts, new_chain.exon_ends, [new_chain.start, new_chain.start], system=self.system)
    ...
    def __str__(self):
        ...
        out = (-1, self.name, self.chrom, self.strand, self.start, self.end,cds_start, cds_end, self.exon_count, exonstarts, exonends, str(self.score), self.gene_id, self.origin, self.origin_spliced, exonframes)
        return "\t".join([str(o) for o in out])
    ...
    def noncirc_CDS(self):
        ...
        return Transcript(nc_name, nc.chrom, nc.strand, nc.exon_starts, nc.exon_ends, (nc.start, nc.end), system=self.system)
        ...
        ##print tx.name
            ...
            #orfs = sorted([ (len(orf),orf,start,end) for (start,end,orf) in all_orfs(tx.spliced_sequence) if len(orf) > ORF_thresh],reverse=True)
                ...
                #except AssertionError:
                    #print start,end,end-start,tx.spliced_length,tx.tx_start,tx.tx_end,tx.exon_count,tx.strand
