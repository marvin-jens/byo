...
def categorize(cats):
    ...
    n = float(np.array(list(cats.values()), dtype=int).max())
...
default_hierarchy = [['CDS', 'UTR5', 'UTR3',], ['exon', 'intron']]
...
class AnnotationTrack(object):
    ...
    def _get_parts(self, exon):
        q = self.get_oriented(exon.chrom, exon.start, exon.end, exon.strand, collapse=True)
        if q.status[0]:
            raise ValueError(f"Query returned with error: '{q.status}'")
    ...
    def _classify_parts(self, parts, hierarchy):
        classification = []
        for H in hierarchy:
            m = ''
            for h in H:
    ...
    def annotate_exon(self, exon, hierarchy=default_hierarchy):
        parts = self._get_parts(exon)
        return self._classify_parts(parts, hierarchy)

    def annotate_chain(self, chain, hierarchy=default_hierarchy):
        parts = set()
...
class AnnotationServer(object):
    def __init__(self, realm="annotation"):
        ...
        self._feature_counter = defaultdict(int)
    ...
    def count_features(self, features):
        self._feature_counter['transcript'] += 1
        for f in features:
            self._feature_counter[f.name.rsplit('/')[-1].split('.')[0]] += 1
    ...
    def load_transcripts(self, fname, system='hg19', **kwargs):
        ...
        for fname in sorted(self._feature_counter.keys()):
            fcount = self._feature_counter[fname]
            self.logger.debug(f"{fname} : {fcount}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--realm', type=str, default='hg19/gencode28', help='if running or connecting to server, this must match')
