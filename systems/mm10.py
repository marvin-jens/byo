...
genome = Track(os.path.join(root,"mm10","genome"),GenomeAccessor,system='mm10')

def get_annotation_track(path=os.path.join(root,"mm10","system_annotation"),**kwargs):
...
chr_sizes = {}
try:
    for c in Importer(os.path.join(root,"mm10","chrom.sizes"),skip=[2],descr="## chrom:str \t size:int \t fileName:str"):
