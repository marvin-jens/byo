...
genome = Track(os.path.join(root,"rn5","genome"),GenomeAccessor,system='rn5')

def get_annotation_track(path=os.path.join(root,"rn5","system_annotation"),**kwargs):
    return Track(path,AnnotationAccessor,**kwargs)

#def get_refGenes(path = os.path.join(root,'annotation','flyBaseGene.ucsc')):
def get_refGenes(path = os.path.join(root,"rn5",'annotation','ensGene.ucsc')):
    from byo.gene_model import transcripts_from_UCSC
    gene_names = {}
    for l in file(os.path.join(root,"rn5",'annotation','gene_names')):
        k,v = l.split('\t')
...
chr_sizes = {}
try:
    for c in Importer(os.path.join(root,"rn5","chrom.sizes"),skip=[2],descr="## chrom:str \t size:int \t fileName:str"):
